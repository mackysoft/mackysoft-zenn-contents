...
この統合では、`SceneEntryPointBase`（`ISceneEntryPoint`）を継承する代わりに、`SceneLifecycleBase`(`ISceneLifecycle`)を継承した型をDIコンテナに登録し、Navigathenaで定義済みの`ScopedSceneEntryPoint`をシーンに配置することで、シーンのライフサイクルをコンポーネント（`MonoBehaviour`）から切り離すことができます。（VContainerのコンテナが親を決めるタイミングの仕様上、少しハッキーですが`LifetimeScope`のGameObjectは非アクティブにしておく必要があります。`ScopedSceneEntryPoint`のインスペクターに表示されている`Create LifetimeScope`ボタンから簡単に生成・設定が出来ます）
...
- 依存性を`readonly`にすることが可能であるため、不変性を保証することが可能になります。不正な書換えによる予期せぬ不具合を防止することができます。
...
UnityのプロジェクトでいざDIを導入しようとしたら「DI分からない～」になりがちですが、その原因は「どこを起点に作っていけば分からない」に集約されると考えています。「Navigathena + DI」は構造的にもかなり扱いやすくなっているのでぜひ！
